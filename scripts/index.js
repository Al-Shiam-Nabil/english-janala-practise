...
const showSynonyms = (words) => {
    const badge = words.map((word) => {
        const htmlElement = `<div class="badge badge-soft badge-success py-4 px-3 mr-2 text-gray-700">${word ? word : "--"
            }  </div>`;
        return htmlElement;
    });
...
<h3 class="text-2xl font-bold">${data.word ? data.word : "--"
        }  (<i class="fa-solid fa-microphone-lines"></i> : ${data.pronunciation ? data.pronunciation : "--"
        } )</h3>
<div class="">
  <p class="font-medium">Meaning</p>
<h3 class="font-bangla text-lg font-semibold">${data.meaning ? data.meaning : "--"
        } </h3>
</div>
<div class="">
...
<div class="bg-white p-8 rounded-xl shadow-sm space-y-4">
  <h3 class="font-bold text-lg">${word.word
                ? word.word
                : `শব্দ পাওয়া যায়নি
`
            }</h3>
  <p>Meaning /Pronounciation</p>
  <h3 class="text-gray-600 font-bangla font-medium">"${word.meaning
                ? word.meaning
                : `অর্থ পাওয়া যায়নি
`
            } / ${word.pronunciation
...
document.getElementById('search-btn').addEventListener('click', () => {
    ...
    //   load all word
    fetch('https://openapi.programming-hero.com/api/words/all').then(res => res.json()).then(words => {
